%% Handwritten string digit Recognition Using Neural Networks
...
clear
clc
addpath(genpath('C:\Users\surya\Desktop\MatlabCentralUpload\testImages'))
...
%im2bw converts grayscale to binary
binaryImage = ~im2bw(grayImage,threshold);
% Removes all object containing fewer than 30 pixels
%background wide area open removes all pixels less than 30 pixels
...
% gaussianfilt is used to smoothen the grayscale image
% padsize is a vector of positive integers that specifies both the amount of padding to add and the dimension along which to add it
% figure(3);
for n=1:Ne
    [r,c] = find(L==n);
